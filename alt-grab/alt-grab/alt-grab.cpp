...
LRESULT CALLBACK WndProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_USER_SHELLICON:
		...
		case WM_RBUTTONDOWN: //right click on tray icon
			...
			InsertMenu(hPopMenu, 0xFFFFFFFF, uFlag, ID_WINDOWCONTROLGROUPS_EXIT, "Quit");
	...
	case WM_COMMAND:
		...
		switch (wmId)
		{
		case ID_WINDOWCONTROLGROUPS_EXIT:
		{
			wantsExit = true;
			Shell_NotifyIcon(NIM_DELETE, &iconData); wantsExit = true;  break;
...
bool InitWin32TrayApp(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	wndHdl = CreateWindow(APP_NAME, APP_NAME, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, NULL, NULL, hInstance, NULL);
	...
	iconData.hIcon = (HICON)LoadIcon(GetModuleHandle(NULL), MAKEINTRESOURCE(IDI_ICON2));
	const char toolTip[128] = APP_NAME;
